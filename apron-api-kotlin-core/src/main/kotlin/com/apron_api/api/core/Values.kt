...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw ApronApiInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ApronApiInvalidDataException("'${name}' is not set")
            is JsonNull -> throw ApronApiInvalidDataException("'${name}' is null")
            else -> throw ApronApiInvalidDataException("'${name}' is invalid, received ${this}")
        }
